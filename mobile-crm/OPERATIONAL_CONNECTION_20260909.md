## 최신 상태 — 2026-09-09 10:04 KST

이 항목이 아래 이전 이력보다 우선한다.

- 업체별 발송 설정 API/Android UI 배포: Git `0aa382ff59657e5a0716e8f960363d52772a5544`, CRM Worker `210569d8-5cd2-4e13-b557-6630cbb5becc`, migration0055 적용. Vercel `dpl_7uRGbFk7T6b45r3iUhv896GwwJH2` READY.
- 전체 회귀284 passed; 후속 발송 대상 테스트19 passed. SENS 조회200. 고객 발송 테스트는 수행하지 않았다.
- 플랫폼 관리자가 회사별 SENS/알림톡 자격과 승인 문구를 등록하고 활성화하면 향후 대상에 자동 적용된다. 알림톡 실자격 등록은 사용자 결정으로 후속 작업이다. Dayone SMS 자격은 등록됐지만 고객 발송 비활성 및 운영 담당 연락처 미등록 상태다.
- 실물 SM-S928N에0.1.1 설치 완료. 후속0.1.2는 입력창 높이와 로그인/OTP 배치를 수정했다. 모든 EditText가 공통 input() 사용, 일반56dp/로그인OTP60dp/멀티라인112dp 최소 높이. setSingleLine 이후에도 View 최소 높이를 보존한다.
-0.1.2 Debug/Release build+lint 성공. 에뮬레이터 실제 검색/분석56dp, 이메일/OTP60dp, 로그인 버튼56dp 측정. 화면 캡처 `.tools/qa/52`–`56` 계열. 실물0.1.2 업데이트/운영 로그인/푸시와 최종 APK 메일은 아직 진행 중이다.
