...
| `AnalyticsSnapshots` | GA4/GSC 유입통계 스냅샷 | (RangeKey, StartDate, EndDate, Source, CreatedAt) |
| `AdminSettings` | 관리자 화면 설정값             | id                                |
| `VisitorIpEvents` | 방문자 IP 기반 접속위치 이벤트 | DayKey/HourKey, LocationKey, IpHash |
| `VisitorLocationHourly` | 접속위치 시간대별 집계 | DayKey, Visits                    |
| `VisitorLocationIpHourly` | 시간대별 고유 IP 중복 방지 | (HourKey, LocationKey, IpHash) |
...
- 2026-05-12: 관리자 유입통계 스냅샷 `AnalyticsSnapshots` 추가. GA4/Search Console 원천 JSON은 R2 `analytics/snapshots/`에 저장하고, 화면용 요약 payload는 D1에 보존.
- 2026-05-12: 관리자 통계 목표량 같은 화면 설정값 저장용 `AdminSettings` 추가.
- 2026-05-14: 방문자 접속위치 집계용 `VisitorIpEvents`, `VisitorLocationHourly`, `VisitorLocationIpHourly` 추가. 관리자 화면에는 접속위치 상위 5곳과 상세 모달용 기간별 누적/월별 일자/마스킹 IP 로그만 노출하고, 원본 이벤트 JSON은 R2 `analytics/ip-checks/`에 보존.
